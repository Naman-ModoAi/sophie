...
export default function ReferralDashboard({ userId }: { userId: string }) {
  ...
  const shareViaEmail = () => {
    if (!referralData) return;
    const subject = encodeURIComponent('Try MeetReady - Get 3 bonus meetings!');
    const body = encodeURIComponent(
      `Hey! I've been using MeetReady to automatically prep for every meeting – it saves me hours each week.\n\nIf you take a lot of external meetings, you'll love it. Sign up with my link and get 3 bonus meetings:\n\n${referralData.referral_link}`
    );
  ...
  const shareViaTwitter = () => {
    ...
    const text = encodeURIComponent(
      `I've been using MeetReady to automatically prep for every meeting. Check it out: ${referralData.referral_link}`
    );
