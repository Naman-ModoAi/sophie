...
-- Date: 2026-01-29
-- Description: Insert default Free and Pro plans

-- Insert default plans (use ON CONFLICT to make migration idempotent)
INSERT INTO public.plans (
  ...
  (
    ...
    1,
    NULL,
    NULL, -- Update with actual Stripe price ID when created
    2000,
...
-- Initialize credits for existing users based on their current plan_type
-- Only update users who don't have credits set yet
DO $$
DECLARE
  v_free_plan_id UUID;
  v_pro_plan_id UUID;
...
BEGIN
  ...
  -- This ensures backward compatibility with the old system
  ...
  SELECT
    ...
    END,
    CASE
      WHEN u.stripe_subscription_status = 'active' THEN 'active'
      WHEN u.stripe_subscription_status = 'canceled' THEN 'canceled'
      WHEN u.stripe_subscription_status = 'past_due' THEN 'past_due'
      ELSE 'active'
    END,
